...
class Source(pykka.ThreadingActor):
    def __init__(self, expression: str, client: QapioGrpcInstance, actor_ref: pykka.ActorRef, *_args: Any, **_kwargs: Any):
        ...
        if "(" in expression:
            client.source(expression).subscribe(lambda x: x)
        else:
            client.source(self.__client.get_manifest().inlet(expression)).subscribe(lambda x: actor_ref.tell(Event(expression, x)))
