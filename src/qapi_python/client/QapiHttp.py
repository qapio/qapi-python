...
class DatasetBuilder:
    ...
    def AddTimeSeries(self, key, endpoint, bucket, measurements, fields, from_date, to_date):
        ...
        self.__items.append({'key': key, 'type': 'timeseries', 'expression': f"Source.Single({json.dumps({'measurements': measurements, 'fields': fields, 'fromDate': from_date.strftime('%Y-%m-%dT%H:%M:%SZ'), 'toDate': to_date.strftime('%Y-%m-%dT%H:%M:%SZ'), 'bucket': bucket})}).Via({endpoint}(6))"})
